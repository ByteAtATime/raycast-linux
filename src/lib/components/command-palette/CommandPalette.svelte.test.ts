...
import { OauthGetTokensMessageSchema, type PluginInfo } from '@raycast-linux/protocol';
...
vi.mock('path', () => ({
	default: {
		dirname: (str: string) => str.substring(0, str.lastIndexOf('/'))
	}
}));
...
describe('CommandPalette.svelte', () => {
	...
	describe('3. Item Selection and Execution (`Enter` key)', () => {
		...
		it('should launch a selected application on Enter', async () => {
			...
			expect(invoke).toHaveBeenCalledWith('launch_app', { exec: mockApps[0].exec });
		...
		it('should copy the calculator answer on Enter', async () => {
			...
			expect(clipboard.writeText).toHaveBeenCalledWith('4');
		});
		...
		it('should execute a simple quicklink (without argument) on Enter', async () => {
			...
			expect(invoke).toHaveBeenCalledWith('execute_quicklink', {
	...
	describe('5. Keyboard Shortcuts and Actions', () => {
		...
		it('should trigger "Copy App Path" with Ctrl+Shift+. for an app', async () => {
			...
			expect(clipboard.writeText).toHaveBeenCalledWith('/usr/bin/test-app-1');
		});
